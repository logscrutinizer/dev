...
{
    return (CPlugin_DLL_API*)new CPlugin_Example_1;
}
...
//----------------------------------------------------------------------------------------------------------------------
void CPlot_Example_1::pvPlotRow(const char* row_p, const unsigned int* length_p,
    ...
    if (m_parser.Search("Time:", 5)) {
        ...
        if (status && m_parser.Search("Value:", 6)) {
            ...
            if (status) {
                ...
                m_valueGraph_p->AddLine((double)m_prevTime, (float)m_prevValue,
                    (double)time, (float)value, rowIndex);
