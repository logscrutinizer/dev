...
void CPlot_Example_4::pvPlotRow(const char* row_p, const unsigned int* length_p, unsigned int rowIndex)
{
  ...
  */
  ...
  if (m_parser.Search("lifeline", 8)) {
    ...
    m_lifeLines_a[lifeLineDest] = m_sequenceDiagram_p->AddLifeLine((float)lifeLineDest - 0.25f,
                                                                   (float)lifeLineDest + 0.25f,
                                                                   lifeLineName,
                                                                   (unsigned int)strlen(lifeLineName),
                                                                   Q_RGB(0x55,
                                                                         20 + 20 * lifeLineDest,
                                                                         20 + 20 * lifeLineDest));
    return;
