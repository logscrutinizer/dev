...
typedef struct
{
    ...
    float                 relative_X;           // 0.0, or in case the label should not be located in the middle of the
                                                // line this parameter shall be used to relatively offset.  0.1 means
                                                // 10% from the left/start, and 0.6 means 60% from the left.. that is
                                                // slightly offset from the middle.
...
{
    GraphicalObject_t         go;
    unsigned int              row2; // As a box may come out of two rows
...
public:
    ...
    unsigned int          count(void) { return m_items; }
...
public:
    ...
    inline unsigned char*    AddBytes(unsigned int size);

    /* Function:    GetBytes
       Description: Used to manage e.g. graphical objects in byte streams. Retrieve the next bytes from the byte stream.
    ...
    inline unsigned char*    GetBytes(void);
...
private:
    ...
    unsigned char*  m_byteStream_p; // The byte stream;
    unsigned char*  m_ref_p; // The current location in the byte stream m_byteStream_p <= m_ref <= m_end_p
    unsigned char*  m_end_p; // Points at the last byte in the stream
...
public:
    ...
    unsigned char* AddBytes(unsigned int size);

    unsigned char* AddBytes_ThreadSafe(unsigned int size); // Add bytes thread safe
...
{
public:
    CGraph_Internal(const char* name_p, unsigned int subPlotID, unsigned int estimatedNumOfObjects);
    ...
    void GetOverrides(bool* isOverrideColorSet_p, unsigned int* overrideColor_p,
...
protected:
    ...
    unsigned int  m_subPlotID; // In-case several graphs shall be given similar
                               // presentation such as color they must have the same graphGroup
...
public:
    CSubPlot(const char* title_p, unsigned int subPlotID, const char* Y_AxisLabel_p);
    virtual ~CSubPlot();
    ...
    inline bool GetGraphs(CList_LSZ** graphsList_pp) {
        *graphsList_pp = &m_graphs; return (m_graphs.isEmpty() ? false : true);
    }
    ...
    inline void SetProperties(unsigned int properties) {
    ...
    inline unsigned int GetProperties(void) {
        return m_properties;
    }
    ...
    */
    CGraph*                             AddGraph(const char* name_p, unsigned int estimatedNumOfObjects = 1024);

    CDecorator*                         AddDecorator(void);
    CSequenceDiagram*                   AddSequenceDiagram(const char* name_p, unsigned int estimatedNumOfObject = 1024);

protected:

    // Do not try to use this function, use the corresponding in CPlot
    unsigned int                        AddLabel(const char* label_p, const unsigned int labelLength);
    ...
    unsigned int                        m_properties; // see SUB_PLOT_PROPERTY_XXX
