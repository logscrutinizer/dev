...
{
public:
    CGraph(const char* name_p, unsigned int subPlotID, unsigned int estimatedNumOfObjects)
    ...
    */
    bool AddLine(double x1, float y1, double x2, float y2, unsigned int row, const char* label_p,
        unsigned char labelLength, int lineColorRGB, float relative_X, unsigned int lineEnds = 0);
   ...
   Parameters
     ...
     Input:         lineEnds       Adds arrow to the line end, default none. Use these in the
                                   ...
                                   PROPERTIES_BITMASK_LINE_ARROW_SOLID_START,
                                       PROPERTIES_BITMASK_LINE_ARROW_OPEN_END PROPERTIES_BITMASK_LINE_ARROW_SOLID_END
    */
    bool AddLine(double x1, float y1, double x2, float y2, unsigned int row, unsigned int labelIndex, int lineColorRGB,
        float relative_X, unsigned int lineEnds = 0);
    ...
    */
    bool AddBox(double x1, float y1, unsigned int row, double x2, float y2, unsigned int row2);
   ...
   Parameters
     ...
     NOTE: if x2,y2 isn't the upper right corner the box might not be displayed.
    */
    bool AddBox(double x1, float y1, unsigned int row, double x2, float y2, unsigned int row2, const char* label_p,
   ...
   Description: Manual override of the LogScrutinizer line pattern enumeration of graphs. This can be used if the plugin
...
{
public:
    CSequenceDiagram(const char* name_p, unsigned int subPlotID, CDecorator* decorator_p,
        unsigned int estimatedNumOfObjects)
   ...
   Description:   Draws a box with specified label and fill color to the left. The life line box will be drawn left most,
    ...
    */
    lifeLine_h AddLifeLine(float y1, float y2, const char* label_p, unsigned char labelLength, int colorRGB);


    /* Function: AddMessage
   Description: This function adds a message from lifeLine1 to lifeLine2, it will result in a line with open arrow head
    ...
    */
    bool AddMessage(lifeLine_h lifeLine1, double x, lifeLine_h lifeLine2, unsigned int row, unsigned int labelIndex,
        int colorRGB, bool synched = false, bool startsExecution = false);

    bool AddMessage(lifeLine_h lifeLine1, double x, lifeLine_h lifeLine2, unsigned int row, const char* label_p,
        unsigned char labelLength, int colorRGB, bool synched = false, bool startsExecution = false);
   ...
   Parameters
     ...
     Input:         startsExecution   If false, the line will be drawn to the life line, otherwise it will end in
                                      y-led where a corner of the execution box would be added
    */
    bool AddReturnMessage(lifeLine_h lifeLine1, double x, lifeLine_h lifeLine2, unsigned int row,
        unsigned int labelIndex, int colorRGB, bool fromExecution = false,
        bool startsExecution = false);

    bool AddReturnMessage(lifeLine_h lifeLine1, double x, lifeLine_h lifeLine2, unsigned int row, const char* label_p,
        unsigned char labelLength, int colorRGB, bool fromExecution = false, bool startsExecution = false);


    /* Function: AddEvent
   Description: This is an event that occurs from a "life line" that doesn't exist in the graph. It will be drawn as an
   ...
   Parameters
     ...
     Input:         startsExecution   If false, the line will be drawn to the life line, otherwise it will end in
                                      y-led where a corner of the execution box would be added
    */
    bool AddEvent(lifeLine_h lifeLine, double x, unsigned int row, unsigned int labelIndex, int colorRGB,
        bool startsExecution = false);
    bool AddEvent(lifeLine_h lifeLine, double x, unsigned int row, const char* label_p, unsigned char labelLength,
        int colorRGB, bool startsExecution = false);

    /* Function: AddExecution
   Description: This displays a box on the life line, showing processing time. The height of the execution box will be
                equal to the height of the life line
   Parameters
     Input:         x                 At which time the message occur (in seconds)
     Input:         lifeLine          Which lifeLine that shall be assigned the processing
    */
    bool AddExecution(lifeLine_h lifeLine, double x1, double x2, unsigned int row, unsigned int labelIndex,
        int colorRGB);
    bool AddExecution(lifeLine_h lifeLine, double x1, double x2, unsigned int row, const char* label_p,
