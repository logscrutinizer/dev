...
/*----------------------------------------------------------------------------------------------------------------------
 ...
 Description: Base class for decoders.
              ...
              be entirely exchanged, or just made longer with the decoded values at the end. Note however that the string
...
{
public:
    CDecoder(const char* matchString, unsigned int uniqueID)
    {
    ...
    /* Function:    pvDecode
     ...
     Parameters
     Input/Output: row_p          A text row from the log. The content of the text row may be modfied, shrinked, and/or
                                  increased up to 64kB (maxLength)
     Input/Output: length_p       The current length of the text row (exluding EOL), and shall be updated to reflect the
                                  changes done to row_p
     Input:        maxLength      If the decoder modifies the string it shall not become larger than this value (inlcuding
                                  the terminating EOL)
    */
    virtual bool pvDecode(char* row_p, unsigned int* length_p, const unsigned int maxLength) = 0;   // PURE VIRTUAL, must be implmented

    // INTERNAL, Do not use
    bool Decode(char* row_p, unsigned int* length_p, const unsigned int maxLength) {
    ...
    inline char* GetMatchString(unsigned int* length_p) {
        *length_p = m_length; return m_matchString;
    }
...
public:
    ...
    */
    virtual bool vPlotExtractTime(const char* row_p, const unsigned int length, double* time_p) {
        *time_p = 0.0; return false;
    }
  ...
  */
    ...
    */
    unsigned int RegisterSubPlot(const char* title_p, const char* Y_AxisLabel);
    ...
    */
    CGraph* AddGraph(unsigned int subPlotID, const char* name_p, unsigned int estimatedNumOfObjects = 1024);
    ...
    */
    CSequenceDiagram* AddSequenceDiagram(
        unsigned int subPlotID, const char* name_p,
        unsigned int estimatedNumOfObjects = 1024);
    ...
    */
     ...
     */
     unsigned int AddLabel(const unsigned int subPlotID, const char* label_p, const unsigned int labelLength);
};
...
protected:
    ...
    */
    void SetPluginFeatures(const unsigned int supportedFeatures) {
...
public:
    bool GetDecoders(CList_LSZ** list_pp) {
        *list_pp = &m_decoders; return (m_decoders.isEmpty() ? false : true);
    }

    bool GetPlots(CList_LSZ** list_pp) {
        *list_pp = &m_plots; return (m_plots.isEmpty() ? false : true);
    }
