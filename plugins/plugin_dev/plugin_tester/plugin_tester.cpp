...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!test_plugin_decoders(dataFile, pluginAPI_p)) throw PLUGIN_TEST_FAIL;
        if (!test_plugin_plotting(dataFile, pluginAPI_p)) throw PLUGIN_TEST_FAIL;
        ...
        case PLUGIN_VERSION_ASSERT: //fall-through
            if (library_p != nullptr) library_p->unload();
        case LIBRARY_LOAD_ASSERT: //fall-through
        {
            if (library_p != nullptr) delete library_p;
        }
        default:
            break;
        } // switch
...
bool test_plugin_decoders(QFile& dataFile, CPlugin_DLL_API *pluginAPI_p)
{
    ...
    if (pluginAPI_p->GetDecoders(&decoderList_p) && !decoderList_p->isEmpty()) {
        ...
        while (!stream.atEnd()) {
            ...
            auto length = static_cast<const unsigned int>(data.length());
