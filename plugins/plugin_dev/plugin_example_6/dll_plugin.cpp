...
//----------------------------------------------------------------------------------------------------------------------
void CPlot_Example_6::pvPlotRow(const char* row_p, const unsigned int* length_p, unsigned int rowIndex)
{
    if (!m_graphAdded)
    {
    ...
    if (line || box) {
        ...
        if (status) status = m_parser.ParseInt(&y1);
        if (status) status = m_parser.ParseInt(&x2);
        if (status) status = m_parser.ParseInt(&y2);
